...
class md5():
    __rLUT = [7,12,17,22]*4 + [5,9,14,20]*4 + [4,11,16,23]*4 + [6,10,15,21]*4
    __cLUT = [3614090360, 3905402710,  606105819, 3250441966, 4118548399, 1200080426, 2821735955, 4249261313,
    ...
    def __proc64(self, blk64:bytes):
        ...
        A,B,C,D = self.buff
        for i in range(64):
            if   i//16 == 0: f1 = lambda b,c,d: (b & c) | (~b & d) ; f2 = lambda i: i
            elif i//16 == 1: f1 = lambda b,c,d: (d & b) | (~d & c) ; f2 = lambda i: (5*i + 1)%16
            elif i//16 == 2: f1 = lambda b,c,d: b ^ c ^ d ;          f2 = lambda i: (3*i + 5)%16
            elif i//16 == 3: f1 = lambda b,c,d: c ^ (b | ~d) ;       f2 = lambda i: (7*i)%16
            F,G = f1(B, C, D), f2(i)
            rot = A + F + self.__cLUT[i] + int.from_bytes(blk64[4*G : 4*G + 4], 'little')
            newB = (B + self.__rl(rot, self.__rLUT[i])) & 0xFFFFFFFF
            A,B,C,D = D,newB,B,C
